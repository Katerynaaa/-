...
public class aTile extends JPanel implements ActionListener {
    ...
    public void paintComponent(Graphics g) {
        if (tile && !tile_glow && tile_value != 0) {
            ...
            g.fillRect(_x + 1, _y + 1, 58, 58);//прыгает НЕ МЕНЯТЬ
            g.setColor(Color.WHITE);
            g.setFont(setFontOnValue(tile_value));
            if (tile_value != 0) {
                g.drawString(String.valueOf(tile_value), _x + font_offset_x, _y + 40);
            }
        }
        if (tile_glow) {

            g.setColor(Color.getHSBColor(40, 240, 128));
            g.fillRect(_x + 1, _y + 1, 58, 58);//НЕ МЕНЯТЬ
            if (tile) {
                g.setColor(Color.WHITE);
                g.setFont(setFontOnValue(tile_value));
                if (tile_value != 0) {
    ...
    Font setFontOnValue(int value) {
        ...
        font_offset_x = 20;
        return new Font("Arial", Font.PLAIN, 26);
    }
